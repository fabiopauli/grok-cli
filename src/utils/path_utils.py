...
"""
Path utilities for Grok Assistant
...
def normalize_path(path_str: str, config: Config, allow_outside_project: bool = False) -> str:
    ...
    # This ensures Windows-style paths are handled correctly on all platforms
    path_str = path_str.replace('\\', '/')

    # Detect Windows absolute paths (e.g., C:/ or D:/) even on non-Windows systems
    # This is a security check to prevent paths like "C:/Windows/..." from being treated as relative
    is_windows_absolute = len(path_str) >= 2 and path_str[1] == ':' and path_str[0].isalpha()
