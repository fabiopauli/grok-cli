...
"""
Text processing utilities for Grok Assistant
...
def get_context_usage_info(
    conversation_history: list[dict[str, Any]], model_name: str, config: Config
) -> dict[str, Any]:
    """
    Get detailed context usage information with safety buffer.
    ...
    buffer = getattr(config, 'token_buffer_percent', 0.10)
    effective_max = int(max_tokens * (1 - buffer))

    usage_percent = (estimated_tokens / effective_max) * 100
    ...
    return {
        ...
        "token_buffer_percent": buffer * 100,
...
def extract_code_blocks(text: str, language: str | None = None) -> list[dict[str, str]]:
    ...
    for lang, content in matches:
        if language is None or lang == language:
            code_blocks.append({"language": lang or "text", "content": content.strip()})
