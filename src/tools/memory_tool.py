...
class MemoryTool(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if "type" not in args:
                return ToolResult.error("Missing required argument: type")
            ...
            # Validate content
            if not content:
                return ToolResult.error("Memory content cannot be empty")
            
            # Validate memory type
            valid_types = ["user_preference", "architectural_decision", "important_fact", "project_context"]
            if memory_type not in valid_types:
                return ToolResult.error(f"Invalid memory type. Must be one of: {', '.join(valid_types)}")
            
            # Validate scope
            valid_scopes = ["directory", "global"]
            if scope not in valid_scopes:
                return ToolResult.error(f"Invalid scope. Must be one of: {', '.join(valid_scopes)}")
            ...
            return ToolResult.error(f"Failed to save memory: {str(e)}")
...
class ListMemoriesTool(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not memories:
                return ToolResult.success(f"No {scope} memories found")
            ...
            return ToolResult.error(f"Failed to list memories: {str(e)}")
...
class RemoveMemoryTool(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not memory_id:
                return ToolResult.error("Memory ID cannot be empty")
            
            # Remove the memory
            success = self._memory_manager.remove_memory(memory_id)
            
            if success:
                return ToolResult.success(f"✓ Removed memory: {memory_id}")
            else:
                return ToolResult.error(f"Memory not found: {memory_id}")
            
        except Exception as e:
            return ToolResult.error(f"Failed to remove memory: {str(e)}")
