    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if self.config.compact_tool_results:
                # Compact format: just filename and content
                return ToolResult.ok(f"{Path(norm_path).name}:\n{read_result['content']}")
            else:
...
class ReadMultipleFilesTool(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # Use compact JSON when compact_tool_results enabled
        indent = None if self.config.compact_tool_results else 2
        separators = (',', ':') if self.config.compact_tool_results else (', ', ': ')
        return ToolResult.ok(json.dumps(response_data, indent=indent, separators=separators))
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if len(content) > self.config.max_file_content_size_create:
                return ToolResult.fail(f"Error: Content too large ({len(content)} chars). Maximum allowed: {self.config.max_file_content_size_create} chars.")

            # Validate Python syntax before creating file
            path = Path(norm_path)
            if path.suffix in [".py", ".pyw"]:
                from ..utils.code_inspector import validate_python_syntax

                syntax_valid, syntax_error = validate_python_syntax(content)
                if not syntax_valid:
                    return ToolResult.fail(
                        f"Cannot create file '{norm_path}' - invalid Python syntax:\n{syntax_error}\n\n"
                        f"The file was NOT created. Please fix the syntax error and try again."
                    )
            ...
            return ToolResult.fail(f"Error creating file '{args.get('file_path', 'unknown')}': {str(e)}")
