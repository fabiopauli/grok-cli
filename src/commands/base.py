...
"""
Base command pattern classes for Grok Assistant
...
class CommandResult:
    ...
    @classmethod
    def ok(cls, message: str = None, data: Any = None, should_continue: bool = True) -> 'CommandResult':
        """Create a successful result."""
        return cls(success=True, message=message, data=data, should_continue=should_continue)

    @classmethod
    def fail(cls, message: str, data: Any = None, should_continue: bool = True) -> 'CommandResult':
...
class CommandRegistry:
    ...
    def find_command(self, user_input: str) -> BaseCommand | None:
        """
    ...
    def execute_command(self, user_input: str, session: GrokSession) -> CommandResult | None:
        """
    ...
    def get_all_command_patterns(self) -> list[str]:
        """
        ...
        """
        return [cmd.get_pattern() for cmd in self.commands]

    def find_similar_command(self, user_input: str, threshold: int = 70) -> str | None:
        """
        Find a similar command to the user input using fuzzy matching.
        ...
        Returns:
            Most similar command pattern or None
        """
        # Only check if input looks like a command (starts with /)
        if not user_input.strip().startswith("/"):
            return None

        # Extract the command part (first word)
        command_part = user_input.strip().split()[0].lower()

        # Try fuzzy matching if available
        try:
            from thefuzz import fuzz, process

            patterns = self.get_all_command_patterns()
            # Find best match
            result = process.extractOne(command_part, patterns, scorer=fuzz.ratio)
            ...
            for pattern in patterns:
                if pattern.startswith(command_part[:3]) and pattern != command_part:
                    return pattern
